...
# ACERTANDO USUARIO E GRUPO DOS ARQUIVOS NOS DIRETORIOS, MOVIMENTANDO ARQUIVOS PARA DIRETORIO DE INSTALACAO, INITTAB E START DO CLIENT
  ...
  chown bin:bin /tmp/libiconv.a
  if [ $? -eq 0 ]
    then
    echo "Ajustado usuario e grupo da biblioteca libiconv.a para bin:bin"
  else
    echo "Falha ao ajustar usuario e grupo da biblioteca libiconv.a"
  fi
  ...
  chmod 444 /tmp/libiconv.a
  if [ $? -eq 0 ]
    then
    echo "Ajustado permissao da biblioteca libiconv.a para 444"
  else
    echo "Falha ao ajustar permissao da biblioteca libiconv.a"
  fi
  ...
  if [ -e /usr/lib/libiconv.a ]
    ...
    rm -rf /tmp/libiconv.a
  else
    mv /tmp/libiconv.a /usr/lib/
    if [ $? -eq "0" ]
      then
      echo "Arquivo libiconv.a movido com suceso para o diretorio /usr/lib"
    else
      echo "Falha ao mover arquivo libiconv.a para o diretorio /usr/lib"
    fi
  fi
